...
ISDStatisticsDestDir = 'D:\NOAA\ISD_stat_corr';
    ...
    for j=1:size(hourFiles,1)
        ...
        for k=1:size(dataArray,2)
            ...
            if (~isempty(values(~isnan(values))))
                ...
                [corrs([num2str(k),' ','0']), pVals([num2str(k),' ','0'])] = corr(E(~isnan(values)),...
                                                                                  values(~isnan(values)));
                ...
                for d=-1:-1:-5                    
                    ...
                    % If E_tmp is empty, set coefficients to NaN
                    if (isempty(E_tmp))
                        corrs([num2str(k),' ',num2str(d)]) = NaN;
                        pVals([num2str(k),' ',num2str(d)]) = NaN;
                    else
                        [corrs([num2str(k),' ',num2str(d)]), pVals([num2str(k),' ',num2str(d)])] = corr(E_tmp', prev_values_tmp');
                    end
                end
            else
                % If column is empty, set coefficients to NaN
                for d=0:-1:-5
                    corrs([num2str(k),' ',num2str(d)]) = NaN;
                    pVals([num2str(k),' ',num2str(d)]) = NaN;
                end
        ...
        for k=1:size(dataArray,2)
            ...
            for d=0:-1:-5
                fprintf(hourFile,'%s',[num2str(k),...
                                       ',',...
                                       num2str(d),...
                                       ',',...
                                       num2str(corrs([num2str(k),' ',num2str(d)])),...
                                       ',',...
                                       num2str(pVals([num2str(k),' ',num2str(d)])),...
                                       char(newline)]);
            end                       
        end
        fclose(hourFile); 
