...
dTvRootDir = 'D:\NOAA\IGRA_PdT_from_2000';
...
for i=1:size(stationdirs,1)
    ...
    for j=1:size(monthdirs,1)
        ...
        for k=1:size(hourFiles)
            ...
            values = textscan(fgetl(statfile),'%s\t%s\t%s\n');      
            fclose(statfile);            
            m = m+1;
            ...
            keySet2{m} = [stationMap(stationdirs(i).name).lat ...
                          ...
                          monthdirs(j).name ...
                          fileNameParts{1}];    
            valueSet2{m} = struct('dTv',str2double(values{1}),'E',str2double(values{2}));
        end        
    end
end
...
%             Z24 = F(X,Y);     
            ...
            for m=1:numel(P0)
                dTvstruct = dTvMap([num2str(LAT0(m),'%.4f') ...
                                    num2str(LON0(m),'%.4f') ...
                                    currentdaydir.name(1:2) ...
                                    currentdayhourfile.name(1:2)]);                
                [Z0_SLP(m),dZ0(m)] = SLP_to_z500_grad(P0(m), ELEV0(m), TV0(m), dTvstruct.dTv, dTvstruct.E);
            end
            for m=1:numel(P24)
                dTvstruct = dTvMap([num2str(LAT24(m),'%.4f') ...
                                    num2str(LON24(m),'%.4f') ...
                                    nextdaydir.name(1:2) ...
                                    nextdayhourfile.name(1:2)]);                
                [Z24_a_SLP(m),dZ24(m)] = SLP_to_z500_grad(P24(m), ELEV24(m), TV24(m), dTvstruct.dTv, dTvstruct.E);
            end            
