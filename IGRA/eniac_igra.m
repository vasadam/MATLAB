...
forecasts = repmat(struct('datetime',[],'Z',[],'MAE_f',[],'MAE_p',[]),2*366*(size(yeardirs,1)-2), 1);
num = 1;
for i=1:size(yeardirs,1)
    if (~(yeardirs(i).isdir) || strcmp(yeardirs(i).name,'.') || strcmp(yeardirs(i).name,'..') ...  % Skip any files, '.' and '..'
        || (str2double(yeardirs(i).name) < 2014) )                   % Skip years before 2000
        continue;
    end
