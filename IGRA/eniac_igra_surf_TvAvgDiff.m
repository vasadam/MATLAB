...
%             Z24 = F(X,Y);     
            ...
            for m=1:numel(P0)
                TvAvgDiffStruct = TvAvgDiffMap([num2str(LAT0(m),'%.4f') ...
                                          num2str(LON0(m),'%.4f') ...
                                          currentdaydir.name(1:2) ...
                                          currentdayhourfile.name(1:2)]);                
                [Z0_SLP(m),dZ0(m)] = SLP_to_z500_TvAvgDiff(P0(m), ELEV0(m), TV0(m), TvAvgDiffStruct.TvAvgDiff, TvAvgDiffStruct.E);
            end
            for m=1:numel(P24)
                TvAvgDiffStruct = TvAvgDiffMap([num2str(LAT24(m),'%.4f') ...
                                                num2str(LON24(m),'%.4f') ...
                                                nextdaydir.name(1:2) ...
                                                nextdayhourfile.name(1:2)]);                
                [Z24_a_SLP(m),dZ24(m)] = SLP_to_z500_TvAvgDiff(P24(m), ELEV24(m), TV24(m), TvAvgDiffStruct.TvAvgDiff, TvAvgDiffStruct.E);
            end            
