...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%   Read the dTv (mean,error) values for each (lat,lon) coordinate pair
dTvRootDir = 'D:\NOAA\IGRA_PdT_from_2000_multilevel';
numStatFiles = size(rdir([dTvRootDir, '\**\*stat.txt']),1);
keySet2 = cell(1,numStatFiles);
valueSet2 = cell(1,numStatFiles);
m=0;
stationdirs = dir(dTvRootDir);
for i=1:size(stationdirs,1)
    if (strcmp(stationdirs(i).name,'.') || strcmp(stationdirs(i).name,'..'))  % Skip '.' and '..'
        continue;
    end    
    
    monthdirs = dir(fullfile(dTvRootDir,stationdirs(i).name));
    for j=1:size(monthdirs,1)
        if (strcmp(monthdirs(j).name,'.') || strcmp(monthdirs(j).name,'..'))  % Skip '.' and '..'
            continue;
        end        
        
        hourFiles = dir(fullfile(dTvRootDir,stationdirs(i).name,monthdirs(j).name));
        for k=1:size(hourFiles)
            if (isempty(strfind(hourFiles(k).name,'stat')))    % Skip files whose name don't contain 'stat'
                continue;
            end
            
            statfile = fopen(fullfile(dTvRootDir,stationdirs(i).name,monthdirs(j).name,hourFiles(k).name));
            dTvStructs = repmat(struct('p',0,'dTv',0,'E',0),1,5); % Usually 5 levels are available (1000, 925, 850, 700, 500)
            line = fgetl(statfile);
            lineNum = 0;
            while(ischar(line))
                lineNum = lineNum+1;
                values = textscan(line,'%s\t%s\t%s\n');
                dTvStructs(lineNum).p = str2double(values{1});
                dTvStructs(lineNum).dTv = str2double(values{2});
                dTvStructs(lineNum).E = str2double(values{3});
                line = fgetl(statfile);
            end
            dTvStructs(lineNum+1:end) = []; 
            fclose(statfile);              
            ...
            keySet2{m} = [stationMap(stationdirs(i).name).lat ...
                          ...
                          monthdirs(j).name ...
                          fileNameParts{1}];    
            valueSet2{m} = dTvStructs;
        end        
    end
end
dTvMap = containers.Map(keySet2, valueSet2);
...
%             Z24 = F(X,Y);     
            ...
            for m=1:numel(P0)
                dTvStructs = dTvMap([num2str(LAT0(m),'%.4f') ...
                                    num2str(LON0(m),'%.4f') ...
                                    currentdaydir.name(1:2) ...
                                    currentdayhourfile.name(1:2)]);                               
                [Z0_SLP(m),dZ0(m)] = SLP_to_z500_grad_multilevel(P0(m), ELEV0(m), TV0(m), dTvStructs);
            end
            for m=1:numel(P24)
                dTvStructs = dTvMap([num2str(LAT24(m),'%.4f') ...
                                    num2str(LON24(m),'%.4f') ...
                                    nextdaydir.name(1:2) ...
                                    nextdayhourfile.name(1:2)]);                
                [Z24_a_SLP(m),dZ24(m)] = SLP_to_z500_grad_multilevel(P24(m), ELEV24(m), TV24(m), dTvStructs);
            end            
