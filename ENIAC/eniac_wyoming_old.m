function [Z,RMS] = eniac_wyoming_core(LAT, LON, Z0, Z24, ...
                                      size_x, size_y, ds, ...
                                      daylen, DtHours)
...
%         Tvavg = Tv-((H500approx-ELEV(x,y))*dT)/2;
...
DDXZ    = zeros(size_x,size_y);     %  x derivative of z
DDYZ    = zeros(size_x,size_y);     %  y derivative of z
D2DX2Z  = zeros(size_x,size_y);     %  second x derivative of z
D2DY2Z  = zeros(size_x,size_y);     %  second y derivative of z
XI      = zeros(size_x,size_y);     %  Laplacian of z
% ETA     = zeros(size_x,size_y);     %  absolute vorticity
DDXETA  = zeros(size_x,size_y);     %  x derivative of eta
DDYETA  = zeros(size_x,size_y);     %  y derivative of eta
DDTZ    = zeros(size_x,size_y);     %  Tendency of z
...
if ( n == 1 )
   % Second x-derivative of z
    D2DX2Z(2:size_x-1,:) = (Z(3:size_x,:)+Z(1:size_x-2,:)-2*Z(2:size_x-1,:))/(ds^2);

    % Second y-derivative of z
    D2DY2Z(:,2:size_y-1) = (Z(:,3:size_y)+Z(:,1:size_y-2)-2*Z(:,2:size_y-1))/(ds^2); 
    
    %%  Laplacian of height
    %%  xi = d2dx2z + d2dy2z;
    XI(2:size_x-1,2:size_y-1) = D2DX2Z(2:size_x-1,2:size_y-1)+D2DY2Z(2:size_x-1,2:size_y-1);
    
    %%  Extend xi to boundaries (to compute Jacobian). xi from BCs in later steps
    XI(1,:) = 2*XI(2,:)-XI(3 ,:);  %  West
    XI(size_x,:) = 2*XI(size_x-1,:)-XI(size_x-2,:);  %  East
    XI(:,1) = 2*XI(:,2)-XI(:,3);  %  South
    XI(:,size_y) = 2*XI(:,size_y-1)-XI(:,size_y-2);  %  North
...
% x-derivative of z
DDXZ(2:size_x-1,:) = (Z(3:size_x,:)-Z(1:size_x-2,:))/(2*ds);

% y-derivative of z
DDYZ(:,2:size_y-1) = (Z(:,3:size_y)-Z(:,1:size_y-2))/(2*ds);

% x-derivative of eta
DDXETA(2:size_x-1,:) = (ETA(3:size_x,:)-ETA(1:size_x-2,:))/(2*ds);

% y-derivative of eta
DDYETA(:,2:size_y-1) = (ETA(:,3:size_y)-ETA(:,1:size_y-2))/(2*ds);
...
%  Eastern boundary
sigE = max(0,-sign(DDYZ(size_x,:)));
DDTXI(size_x,:) = sigE.*(2*DDTXI(size_x-1,:)-DDTXI(size_x-2,:));
...
%  Northern boundary
sigN = max(0,+sign(DDXZ(:,size_y)));
DDTXI(:,size_y) = sigN.*(2*DDTXI(:,size_y-1)-DDTXI(:,size_y-2));
...
ERROR = (Z24(2:size_y-1,2:size_x-1)-Z(2:size_y-1,2:size_x-1)).^2;
RMS = sum(ERROR(:)) / ((size_y-2)*(size_x-2));
